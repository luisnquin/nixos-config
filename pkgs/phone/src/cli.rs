...
When phone.toml names a host, each step runs there over ssh, next to the tree
and the toolchain, and its devices are driven from here. What it remembers
having built stays there too, so a second machine converging the same project
